...
import {
  ...
  Navigation2, Navigation, MapPin, Clock, Gauge, ChevronRight
} from 'lucide-react';
...
const RESCUE_CORRIDORS = [
  { id: 'r1', label: 'Hyderabad → Warangal', from: [17.3850, 78.4867] as [number,number], to: [17.9689, 79.5941] as [number,number], priority: 'HIGH' },
  { id: 'r2', label: 'Vijayawada → Guntur',  from: [16.5062, 80.6480] as [number,number], to: [16.3067, 80.4365] as [number,number], priority: 'CRITICAL' },
  { id: 'r3', label: 'Karimnagar → Nalgonda',from: [18.4386, 79.1288] as [number,number], to: [17.0575, 79.2684] as [number,number], priority: 'CRITICAL' },
  { id: 'r4', label: 'Visakhapatnam → Kakinada', from: [17.6868, 83.2185] as [number,number], to: [16.9891, 82.2475] as [number,number], priority: 'MEDIUM' },
  { id: 'r5', label: 'Kurnool → Tirupati',   from: [15.8281, 78.0373] as [number,number], to: [13.6284, 79.4192] as [number,number], priority: 'HIGH' },
];
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex items-center space-x-2">
          ...
          <h2 className="font-bold text-sm text-slate-800 dark:text-slate-100">Rescue Route Intelligence</h2>
          <span className="ml-2 text-[10px] text-slate-400">OSRM Live Routing API</span>
          {loadingRoutes && (
            <span className="ml-auto text-[10px] text-indigo-400 animate-pulse flex items-center space-x-1">
                    ...
                    {stats ? (
                      <div className="flex items-center space-x-3 text-[10px] text-slate-400">
                        <span className="flex items-center space-x-1">
                          <Navigation size={10} />
                          <span>{stats.distKm > 0 ? `${stats.distKm} km` : 'Calculating…'}</span>
                        </span>
                        <span className="flex items-center space-x-1">
                          <Clock size={10} />
                          <span>{stats.durMins > 0 ? `~${stats.durMins} min` : '—'}</span>
                        </span>
                      </div>
                    ) : (
                      <span className="text-[10px] text-slate-400 animate-pulse">Fetching route…</span>
                    )}
                  </div>
                ...
                <div className="space-y-5 h-full flex flex-col">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-[10px] text-slate-400 mt-0.5">Computed via OSRM Open Source Routing Machine</p>
                    </div>
                    <span className={`px-2 py-1 border rounded-lg text-[9px] font-black uppercase ${priorityColor(c.priority)}`}>{c.priority}</span>
                  </div>
                  ...
                  <div className="space-y-2 flex-1">
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Route Waypoints</p>
                    <div className="space-y-2">
                      {[
                        { label: 'Origin', coord: c.from, dot: 'bg-rose-500' },
                        { label: 'Destination', coord: c.to, dot: 'bg-emerald-500' },
                      ].map(w => (
                    ...
                    <p className="text-[10px] text-slate-400 pt-1">
                      Route drawn on map above — enable <span className="font-bold text-slate-600 dark:text-slate-300">Rescue Routes</span> layer to view polyline.
                    </p>
                  </div>
                </div>
              );
